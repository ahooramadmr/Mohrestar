    ...
    <script>
        ...
        canvas.addEventListener("pointerdown", function(e) {
            ...
            for (let p of players) {
                if (p.team !== currentTurn) continue;
                const dx = x - p.x;
                const dy = y - p.y;
        ...
        function resetPositions() {
            // ریست کردن موقعیت مهره‌ها با ابعاد جدید
            const w = canvas.width;
            const h = canvas.height;
            const blueYStart = (h - 4 * 90) / 2;
            const redYStart = (h - 4 * 90) / 2;
            ...
            for (let p of players) {
                if (p.team === "blue") {
                    p.x = w * 0.25;
                    p.y = blueYStart + blueIdx * 90;
                    blueIdx++;
                } else {
                    p.x = w * 0.75;
                    p.y = redYStart + redIdx * 90;
                    redIdx++;
                }
                p.vx = 0;
                p.vy = 0;
        ...
        function checkGoal() {
            const top = canvas.height / 2 - GAME.goalHeight / 2;
            const bottom = canvas.height / 2 + GAME.goalHeight / 2;
            ...
            // پایان بازی
            if (blueScore >= GAME.winScore || redScore >= GAME.winScore) {
                gameOver = true;
            }
        }
        ...
        function drawField() {
            ...
            // خط وسط
            ctx.beginPath();
            ctx.moveTo(canvas.width / 2, GAME.fieldMargin);
            ctx.lineTo(canvas.width / 2, canvas.height - GAME.fieldMargin);
            ctx.stroke();
            ...
            ctx.lineWidth = 8;
            ctx.beginPath();
            ctx.moveTo(GAME.fieldMargin, canvas.height / 2 - GAME.goalHeight / 2);
            ctx.lineTo(GAME.fieldMargin, canvas.height / 2 + GAME.goalHeight / 2);
            ctx.stroke();

            // دروازه راست
            ctx.beginPath();
            ctx.moveTo(canvas.width - GAME.fieldMargin, canvas.height / 2 - GAME.goalHeight / 2);
            ctx.lineTo(canvas.width - GAME.fieldMargin, canvas.height / 2 + GAME.goalHeight / 2);
            ctx.stroke();
        }
        ...
        function drawDiskShadow(player) {
            ctx.beginPath();
            ctx.arc(player.x + 3, player.y + 4, player.r, 0, Math.PI * 2);
        ...
        function drawScore() {
            ...
            ctx.fillText(blueScore + " : " + redScore, canvas.width / 2, 50);
        }
        ...
        function drawTurn() {
            ctx.fillStyle = "white";
            ctx.font = "24px Arial";
            ctx.fillText("Turn : " + currentTurn, canvas.width / 2, 90);
        }
        ...
        function drawGameOver() {
            ...
            ctx.font = "60px Arial";
            ctx.textAlign = "center";
            const winner = blueScore > redScore ? "BLUE WINS!" : "RED WINS!";
        ...
        function aiMove() {
            ...
            const power = Math.min(GAME.maxPower, dist);
        ...
        function goalEffect(team) {
            const color = team === "blue" ? "#2196F3" : "#F44336";
            createParticles(canvas.width / 2, canvas.height / 2, color);
            shakeScreen();
        }
        ...
        // تنظیم مجدد با تغییر اندازه پنجره
        window.addEventListener("resize", function() {
            resizeCanvas();
            // موقعیت‌ها را دوباره تنظیم کن
            const w = canvas.width;
            const h = canvas.height;
            const blueYStart = (h - 4 * 90) / 2;
            const redYStart = (h - 4 * 90) / 2;
